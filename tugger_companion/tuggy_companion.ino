...
// --- NEW COMPANION LOGIC ---
enum TuggyMood { HAPPY, NEUTRAL, SICK };

const unsigned char tuggy_happy_bits[] = { 0x00, 0xff, 0x00, 0x00, 0xff, 0x00, 0xc0, 0xff, 0x03, 0xc0, 0xff, 0x03, 0xf0, 0x00, 0x0f, 0xf0, 0x00, 0x0f, 0x3c, 0xcf, 0x33, 0x3c, 0xcf, 0x33, 0x3c, 0xc3, 0x30, 0x3c, 0xc3, 0x30, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xf3, 0x00, 0xcf, 0xf3, 0x00, 0xcf, 0x33, 0xff, 0xcc, 0x33, 0xff, 0xcc, 0x33, 0x00, 0xcc, 0x33, 0x00, 0xcc, 0xf0, 0xff, 0x0f, 0xf0, 0xff, 0x0f, 0xf0, 0x00, 0x0f, 0xf0, 0x00, 0x0f, 0xfc, 0xc3, 0x3f, 0xfc, 0xc3, 0x3f };
const unsigned char tuggy_neutral_bits[] = { 0x00, 0xff, 0x00, 0x00, 0xff, 0x00, 0xc0, 0xff, 0x03, 0xc0, 0xff, 0x03, 0xf0, 0x00, 0x0f, 0xf0, 0x00, 0x0f, 0x3c, 0xcf, 0x33, 0x3c, 0xcf, 0x33, 0x3c, 0xcf, 0x33, 0x3c, 0xcf, 0x33, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xf3, 0x00, 0xcf, 0xf3, 0x00, 0xcf, 0x33, 0x00, 0xcc, 0x33, 0x00, 0xcc, 0x33, 0xff, 0xcc, 0x33, 0xff, 0xcc, 0xf0, 0xff, 0x0f, 0xf0, 0xff, 0x0f, 0xf0, 0x00, 0x0f, 0xf0, 0x00, 0x0f, 0xfc, 0xc3, 0x3f, 0xfc, 0xc3, 0x3f };
const unsigned char tuggy_sick_bits[] = { 0x00, 0xff, 0x00, 0x00, 0xff, 0x00, 0xc0, 0xff, 0x03, 0xc0, 0xff, 0x03, 0xf0, 0x00, 0x0f, 0xf0, 0x00, 0x0f, 0x3c, 0xc3, 0x30, 0x3c, 0xc3, 0x30, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xf3, 0x00, 0xcf, 0xf3, 0x00, 0xcf, 0x33, 0xff, 0xcc, 0x33, 0xff, 0xcc, 0x33, 0xc3, 0xcc, 0x33, 0xc3, 0xcc, 0xf0, 0xff, 0x0f, 0xf0, 0xff, 0x0f, 0xf0, 0x00, 0x0f, 0xf0, 0x00, 0x0f, 0xfc, 0xc3, 0x3f, 0xfc, 0xc3, 0x3f };
...
// ==================== COMPANION EVALUATOR ====================
TuggyMood evaluateTuggyMood() {
  if (orderCount == 0 || deviceMode != MODE_TUGGER) return HAPPY;
  
  unsigned long now = millis();
  unsigned long maxAge = 0;
  
  for (int i = 0; i < orderCount; i++) {
    if (orders[i].status == 0) { // Pending
      unsigned long age = now - orders[i].createdTime;
      if (age > maxAge) maxAge = age;
    }
  }
  
  // Rules for mockup: >45m (2700000ms) = SICK, >15m (900000ms) = NEUTRAL
  if (maxAge > 2700000UL) return SICK;
  if (maxAge > 900000UL) return NEUTRAL;
  return HAPPY;
}
...
void updateDisplay(String statusMsg = "") {
  ...
  if (deviceMode == MODE_TUGGER) {
    ...
    // --- DRAW TUGGY COMPANION ---
    TuggyMood mood = evaluateTuggyMood();
    const unsigned char* bits = tuggy_happy_bits;
    if (mood == NEUTRAL) bits = tuggy_neutral_bits;
    else if (mood == SICK) bits = tuggy_sick_bits;
    
    // Position character in header gap (e.g. X=95)
    display.drawXBitmap(95, 0, bits, 24, 24, 0); 
  }
  ...
  display.drawLine(0, 24, 250, 24, 0);
...
void processPacketData(uint8_t *data, size_t len) {
  ...
  if (p->type == PKT_ORDER &&
    ...
    if (orderCount < MAX_ORDERS) {
      ...
      o.status = 0;
      o.createdTime = millis(); // TUGGY UPDATE: Track initialization!
      updateDisplay();
    }
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("Booting E290 Hybrid Firmware with TUGGY COMPANION");
...
void loop() {
  ...
  // Update face periodically to show state transitions
  static unsigned long lastMoodCheck = 0;
  if (millis() - lastMoodCheck > 60000) { 
      lastMoodCheck = millis();
      if (deviceMode == MODE_TUGGER && orderCount > 0) {
          updateDisplay(); // Automatically refreshes the face
      }
  }
}
